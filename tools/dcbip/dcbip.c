...
#define BUFFER_SIZE (64 * 1024)  /* 32 KB */
...
char *read_ip_pull(const char *file) {
    ...
    if (fd) {
        ...
        fprintf(stderr, "An error has been occurred while opening the '%s'\n", file);
        return NULL;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    char *pch = NULL;
    char *strings = read_ip_pull(argv[1]);
    pch = strtok(strings,"\n");

    while (pch) {
        printf("Checking ip diapasone: %s\n", pch);

        strval = NULL;
        found = false;

        snprintf(url, URL_SIZE, URL_FORMAT, pch);
